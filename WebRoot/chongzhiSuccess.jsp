...
<!--top-header-->
  ...
  %>
   <%-- 当前订单id(用于充值成功后继续支付):<%=request.getAttribute("orderId") %><br>
  当前的要购物的金额：<%=request.getAttribute("totalMoney")%><br>
  是否删除当前购物车：<%=request.getAttribute("delCart")%> --%>
    充值成功，<a href="EditTheOrderStatusByCustomer.jsp?orderId=<%=request.getAttribute("orderId")%>&totalMoney=<%=request.getAttribute("totalMoney")%>&delCart=<%=request.getAttribute("delCart")%>">返回支付页面</a>
  </body>
</html>
